...
class User(db.Model):
    ...
    works = db.relationship(
        "Work", backref="author", lazy="dynamic", cascade="all, delete-orphan"
    )
    comments = db.relationship(
        "Comment", backref="author", lazy="dynamic", cascade="all, delete-orphan"
    )
    collections = db.relationship(
        "Collection", backref="collector", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    post_likes = db.relationship(
        "PostLike", backref="user", lazy="dynamic", cascade="all, delete-orphan"
    )
    post_comments = db.relationship(
        "PostComment", backref="author", lazy="dynamic", cascade="all, delete-orphan"
    )
    checkins = db.relationship(
        "Checkin", backref="user", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Work(db.Model):
    ...
    style = db.Column(db.String(50), nullable=True)  # 书法风格：楷书、行书、草书等
    dynasty = db.Column(db.String(50))
    author_name = db.Column(db.String(100))
    author_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    charactr_count = db.Column(db.Integer)
    ...
    # 关系
    comments = db.relationship(
        "Comment", backref="work", lazy="dynamic", cascade="all, delete-orphan"
    )
    collections = db.relationship(
        "Collection", backref="work", lazy="dynamic", cascade="all, delete-orphan"
    )
    likes = db.relationship(
        "Like", backref="work", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self, include_author=True):
        ...
        data = {
            ...
            "charactr_count": self.charactr_count,
            "views": self.views,
            "status": self.status,
...
class Collection(db.Model):
    ...
    # 唯一约束：一个用户不能重复收藏同一作品
    __table_args__ = (
        db.UniqueConstraint("user_id", "work_id", name="unique_user_work_collection"),
    )
...
class Like(db.Model):
    ...
    # 唯一约束：一个用户不能重复点赞同一作品
    __table_args__ = (
        db.UniqueConstraint("user_id", "work_id", name="unique_user_work_like"),
    )
...
class Post(db.Model):
    ...
    # 关系
    likes = db.relationship(
        "PostLike", backref="post", lazy="dynamic", cascade="all, delete-orphan"
    )
    comments = db.relationship(
        "PostComment", backref="post", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Checkin(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "user_id", "checkin_date", name="unique_user_daily_checkin"
        ),
    )
